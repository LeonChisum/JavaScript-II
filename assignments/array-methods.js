...
let upper = (x) => x.first_name.toUpperCase();
let allCaps = runners.map(upper);
...
runners.forEach((x) => ticketPriceTotal.push(`${x.donation}`));
let total = (acc,curr) => acc += curr;
let price = ticketPriceTotal.reduce(total, 0);
